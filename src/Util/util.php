...
namespace KadokWeb\Plates\Util;

use KadokWeb\Plates\Exception\PlatesException;
use KadokWeb\Plates\Exception\StackException;
...
/** wraps a closure in output buffering and returns the buffered content. */
...
function cachedFileExists(Psr\SimpleCache\CacheInterface $cache, $ttl = 3600, $file_exists = 'file_exists')
{
    return function ($path) use ($cache, $ttl, $file_exists) {
        $key = 'KadokWeb.Plates.file_exists.' . $path;
        $res = $cache->get($key);
        if (!$res) {
